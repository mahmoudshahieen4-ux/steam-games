...
const sidebarStyles = {
  container: (open: boolean) =>
    `${open ? "w-64 fixed lg:relative" : "w-0 lg:w-24 relative"} top-[64px] lg:top-0 left-0 lg:inset-auto h-[calc(100vh-64px)] lg:h-full bg-gradient-to-b from-red-950/20 to-black border-r border-red-900/40 overflow-y-auto overflow-x-hidden flex-shrink-0 backdrop-blur-xl z-50 lg:z-40 transition-[width] duration-300 ease-in-out`,
  nav: "p-4 space-y-2",
...
const searchStyles = {
  ...
  input:
    "w-full bg-gradient-to-r from-red-950/30 to-black/50 backdrop-blur-sm border border-red-900/30 text-white placeholder:text-gray-500 pl-12 pr-4 py-4 rounded-xl outline-none focus:ring-2 focus:ring-red-600/50 focus:border-red-600/50 transition-all",
};
...
const featureStyles = {
  ...
  errorContainer:
    "p-8 text-center bg-red-950/20 border border-red-900/30 rounded-xl",
};
...
const dashboardStyles = {
  ...
  widget:
    "bg-gradient-to-br from-red-950/20 to-black/40 border border-red-900/20 rounded-xl p-6 backdrop-blur-sm",
  stickyWidget:
    "bg-gradient-to-br from-red-950/20 to-black/40 border border-red-900/20 rounded-xl p-6 backdrop-blur-sm",
      ...
      try {
        ...
        if (page === 1 && !searchQuery && selectedCategory === "all") {
          ...
          const trending = await fetchGames({
            search:
              "Grand Theft Auto V, Assassin's Creed Valhalla, Red Dead Redemption 2, Cyberpunk 2077",
            page_size: "4",
          });
          setTrendingGames(trending);

          // Best of last year (more likely to have data)
          const bestByYear = await fetchGames({
        ...
        <aside className={sidebarStyles.container(sidebarOpen)}>
          <nav className={sidebarStyles.nav}>
            ...
            {[
              ...
              <motion.div key={item.label} whileHover={{ scale: 1.02, x: 5 }}>
                <Link
                  to={item.path}
                  className={`${sidebarStyles.link} ${
