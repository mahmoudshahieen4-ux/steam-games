...
const headerStyles = {
  container:
    "sticky top-0 bg-gradient-to-r from-black via-red-950/20 to-black border-b border-red-900/30 px-6 py-1 flex items-center justify-between backdrop-blur-md z-[60]",
  ...
  navUnderline:
    "absolute -bottom-2 left-0 w-0 h-0.5 bg-gradient-to-r from-red-600 to-red-500 group-hover:w-full transition-all",
  rightSection: "flex items-center gap-3",
  notificationButton:
    "relative p-2 text-gray-400 hover:text-white transition-colors hover:bg-red-900/20 rounded",
  notificationDot: "absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full",
  settingsButton:
    "p-2 text-gray-400 hover:text-white transition-colors hover:bg-red-900/20 rounded",
  loginButton:
    "px-4 py-2 text-gray-400 hover:text-white transition-colors hidden sm:block",
  signupButton:
    "px-4 py-2 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white rounded font-medium transition-all shadow-lg shadow-red-900/20 active:scale-95 hidden sm:block",
};
    ...
    <header className={headerStyles.container}>
        ...
        <Link to="/">
          ...
          >
            <div className="flex items-center">
              ...
              <h2 className="mr-4">
                STEAM <span className="text-red-600 font-bold">FIRE</span>
              </h2>
            </div>
          </motion.h1>
        </Link>
      ...
      <nav className={headerStyles.nav}>
        ...
        >
          Library
          {!isActive("/library") && (
            <span className={headerStyles.navUnderline} />
          )}
        </Link>
        <Link
          to="/community"
      ...
      <div className={headerStyles.rightSection}>
        ...
        <Link to="/settings" className={headerStyles.settingsButton}>
